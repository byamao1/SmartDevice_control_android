...
/**
 * 通用型组件。可以使得item能够移动位置。
 * 从本质上来说，并没有将图标的view进行移动，而是通过将数据更新再通知GridView更新的方式实现
 * @author xiaanming
 */
public class DragGridView extends GridView{
//	private boolean isDragable = false; 
//
//	public void setDragable(boolean input){
//		isDragable = input;
//	}
...
//	}
    ...
    /**
     * 设置回调接口，在拖动图标（改变了图标顺序）时同步更新数据顺序
    ...
    private Runnable mScrollRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if(moveY > mUpScrollBorder){
                 scrollY = -speed;
                 mHandler.postDelayed(mScrollRunnable, 25);//如果触点一直停在view底部，说明要一直往下翻。这里就是解决它的
            }else if(moveY < mDownScrollBorder){
                scrollY = speed;
                 mHandler.postDelayed(mScrollRunnable, 25);//同上
            }else{
                scrollY = 0;
                mHandler.removeCallbacks(mScrollRunnable);
            }
